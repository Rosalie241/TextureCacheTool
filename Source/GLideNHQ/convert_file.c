...
static size_t image_content_length(const struct GHQTexInfo *file)
{
	size_t size;
...
static int resize_image_bmp(struct GHQTexInfo *file)
{
	...
	if (file->data_size > (UINT32_MAX - header_size)) {
		fprintf(stderr, "Too large texture for bmp export\n");
		return -EPERM;
	}

	buf = malloc(file->data_size + header_size);
	if (!buf) {
	...
	header_v5->filesize = htole32(file->data_size + header_size);
	...
	header_v5->datasize = htole32(file->data_size);
	...
	free(file->data);
	file->data = buf;
	file->data_size += header_size;

	return 0;
}

static int normalize_image_r5g6b5(struct GHQTexInfo *file)
{
	uint32_t *buf;
	uint16_t *data, raw;
	size_t newsize;
	...
	file->data_size = (uint32_t)newsize;
	file->format = GR_BGRA;

	return 0;
}

static int normalize_image_r5g5b5a1(struct GHQTexInfo *file)
{
	uint32_t *buf;
	uint16_t *data, raw;
	size_t newsize;
...
static int normalize_image_r4g4b4a4(struct GHQTexInfo *file)
{
	uint32_t *buf;
	uint16_t *data, raw;
	size_t newsize;
...
static int normalize_image_r8g8b8a8(struct GHQTexInfo *file)
{
	uint32_t *data, raw;
	size_t pixels, pos;
	uint32_t p, a, r, g, b;
...
static int resize_image_content(struct GHQTexInfo *file)
{
	int ret;
...
int prepare_file(struct GHQTexInfo *file)
{
	...
	if (file->format & GR_TEXFMT_GZ) {
		...
		ret = uncompress(buf, &destLen, file->data, file->data_size);
		if (ret != Z_OK) {
			free(buf);
		...
		free(file->data);
		file->data = buf;
		file->data_size = (uint32_t)expected_size;
	} else {
		if (expected_size != file->data_size) {
